...
def Train_With_Stratifiedkfold(VideoSet, VideoType , VideoMap, Label):
    ...
    for train_index, valid_index in skf.split(VideoSet, VideoType):
        ...
        Train_loader = DataLoader(Training_Dataset, batch_size=3, shuffle=True, num_workers=0)  # num_workers代表要用多少個subprocess載入data
        Validate_loader = DataLoader(Validate_Dataset, batch_size=3, shuffle=True, num_workers=0)
...
Training_Video_Path = ["test_inc/*.mp4"]
